...
```mermaid
flowchart LR
  H[Hypothesis + Success criteria] --> D[Design\nmetrics • population • MDE]
  D --> I[Instrumentation\nlogging • exposure • IDs]
  I --> S[Safety gates\nshadow/canary]
  S --> L[Launch\nrandomization + routing]
  L --> M[Monitor\nSRM • guardrails • health]
  M --> A[Analyze\nstats • slices • sanity checks]
  A --> R{Decision}
  R -->|Ship + Ramp| P[Progressive rollout\n+ continued monitoring]
  R -->|Iterate| T[Tune / fix / redesign]
  R -->|Rollback| B[Rollback + postmortem]
```
...
```mermaid
flowchart TB
  U[User Request] --> G[Gateway / Edge]
  G --> R[Experiment Router\nbucketing + eligibility]
  R --> A[Variant A\nControl]
  R --> B[Variant B\nTreatment]

  A --> O[Online Metrics\nlatency • errors • safety]
  B --> O
...
```mermaid
flowchart TD
  Q{Do you need causal\nbusiness impact?} -->|Yes| AB[A/B Test (RCT)]
  Q -->|No| RISK{Is change high risk\nor infra-heavy?}
  RISK -->|Yes| SH[Shadow -> Canary]
  RISK -->|No| CAN[Canary or limited rollout]

  AB --> RANK{Is it ranking/reco?}
  RANK -->|Yes| INT[Consider Interleaving\nfor faster sensitivity]
  RANK -->|No| STD[Standard RCT + guardrails]

  STD --> RAMP[Progressive ramp]
  INT --> RAMP
  SH --> RAMP
  CAN --> RAMP
```
