...
stateDiagram-v2
  ...
  S1 --> S2: Orchestrator + model store\n+ repeatable pipeline
  S2 --> S3: Checkpoints + lineage\n+ stable eval gates
  S3 --> S4: Drift/perf triggers\n+ safe test-in-prod automation
```
...
flowchart TB
  ...
  subgraph DATA[Data & Labels]
    NEW[Fresh data + labels\n(or weak labels)]
    FEAT[Feature pipeline / FS\n(logged serving features)]
  end

  subgraph TRAIN[Training]
    CHAMP[(Champion model)]
    FT[Train from scratch\nor fine-tune]
  ...
  NEW --> FEAT --> FT
  CHAMP --> FT --> CAND
  CAND --> OFF -->|pass| TIP -->|pass| MR --> DEP --> CHAMP
  OFF -->|fail| RB
  TIP -->|fail| RB
```
