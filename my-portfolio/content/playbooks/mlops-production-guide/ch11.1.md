...
```mermaid
flowchart TD
  A[Alert: quality/drift/DQ] --> B{Service health broken?\n5xx/latency/oom}
  B -->|Yes| C[Infra incident:\nrollback, scale, deps, timeouts]
  B -->|No| D{Data quality broken?\nschema/freshness/null spike}
  D -->|Yes| E[Upstream fix:\nETL/stream bug, join break,\nunit change, backfill]
  D -->|No| F{Skew/drift?\nP(X) or P(ŷ) shifted}
  F -->|Yes| G[Trace drift:\nwhich features + which slices\nimpacting outputs]
  G --> H{Perf/KPI impacted?}
  H -->|Yes| I[Mitigate:\n- fallback model/rules\n- retrain/recalibrate\n- adjust thresholds\n- route to specialized model]
  H -->|No| J[Monitor closer:\nadjust thresholds,\nconfirm with labels]
  F -->|No| K[Model bug/regression:\nwrong version, bad config,\npre/post mismatch]
```
