...
flowchart TB
  ...
  GW --> LB[Load Balancer / Routing]
  LB --> MS[Model Service\nREST/gRPC]
  MS --> FS[Online Feature Store/KV\noptional]
  MS --> LOG[Logs/Tracing]
  MS --> MET[Metrics\nlatency, errors, QPS]
  MS --> REG[Model Registry\nartifact + stage]
  REG --> MS
```
...
```mermaid
flowchart LR
  S[Data source\nLake/Warehouse/DB] --> O[Orchestrator\n(schedule/event)]
  O --> J[Batch prediction job\n(load model, score)]
  J --> P[Prediction store\nDB/Warehouse/S3]
  J --> R[Reports\nquality + drift + costs]
  REG[Model Registry] --> J
```
