...
export async function generateMetadata() {
    return {
        title: 'Wealth Management Agent | Product Proposals',
        description: 'Financial advisory with portfolio analysis and compliance guardrails',
    };
}

export default async function WealthManagementAgentPage() {
    const content = await getAgenticAIProductBySlug('flagship_product_proposals/wealth/product_description');

    const uiMockups = [
        { name: 'advisor_dashboard.png', title: 'Advisor Dashboard' },
        { name: 'client_intake_data_collection.png', title: 'Client Intake & Data Collection' },
        { name: 'proposal_generation_review.png', title: 'Proposal Generation & Review' },
        { name: 'proposal_approval_compliance.png', title: 'Proposal Approval & Compliance' },
        { name: 'scenario_analysis_projection.png', title: 'Scenario Analysis & Projection' },
    ];
                    ...
                    <div className="flex-1 max-w-4xl product-body">
                        ...
                        <section className="mb-16">
                            ...
                            <div className="bg-muted/20 rounded-xl p-6">
                                ...
                                <img
                                    src="/agentic-ai-products/flagship_product_proposals/wealth/technical_architecture.png"
                                    alt="Wealth Management Agent Technical Architecture"
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                {uiMockups.map((mockup) => (
                                    <div key={mockup.name} className="bg-muted/20 rounded-xl p-4">
                                        ...
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img
                                            src={`/agentic-ai-products/flagship_product_proposals/wealth/ui_mockups/${mockup.name}`}
                                            alt={mockup.title}
                                            className="w-full h-auto rounded-lg"
                                        />
                                    </div>
