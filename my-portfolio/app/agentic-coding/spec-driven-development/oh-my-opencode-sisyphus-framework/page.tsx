...
export async function generateMetadata() {
    const content = await getContentBySlug('agentic-coding', 'spec_driven_development/oh_my_opencode_sisyphus_framework');

    return {
        title: content?.title || 'OpenCode Sisyphus Framework',
        description: content?.summary || 'Specific orchestration pattern for spec-driven development',
    };
}

export default async function OhMyOpenCodeSisyphusFrameworkPage() {
    const content = await getContentBySlug('agentic-coding', 'spec_driven_development/oh_my_opencode_sisyphus_framework');
