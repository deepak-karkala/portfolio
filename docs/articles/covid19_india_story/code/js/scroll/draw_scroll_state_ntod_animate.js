...
draw_scroll_state_ntod_animate(idname, filename, base_width, base_height);
*/

function draw_scroll_state_ntod_animate(idname, file, width, height, margin) {
    ...
    d3.csv(file, function(error, data) {
		...
		data.forEach(function(d, i) {
			...
			cases_tooltip = +d[end_date_str+"_cases"];
			deaths_tooltip = +d[end_date_str+"_deaths"];
			ntod_tooltip = +d[end_date_str+"_ntod"];
			testspm_tooltip = +d[end_date_str+"_testspm"];
        ...
        .enter().append("circle")
			...
			.on("mouseover", function(d, i) {
			    ...
			    return tooltip.html(`<div class='well'>`+
                                    ...
                                    `Confirmed Cases: <span class="case_count_info">`+ parseInt(d[end_date_str+"_cases"]) +`</span></br>`+
                                    `Deaths: <span class="case_count_info">`+ parseInt(d[end_date_str+"_deaths"]) +`</span></br>`+
                                    `Number of days to double: <span class="case_count_info">`+ parseInt(d[end_date_str+"_ntod"]) +`</span></br>`+
                                    `Tests per million: <span class="case_count_info">`+ parseInt(d[end_date_str+"_testspm"])+`</span>`+
                                    `</div>` )
