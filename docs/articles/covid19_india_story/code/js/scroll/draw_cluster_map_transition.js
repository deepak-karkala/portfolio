...
function draw_cluster_map_transition(idname, filename, width, height) {
	...
	*/
    ...
    function ready(error, world, india, data) {
          	...
          	.attr("class", function(d) {
          		...
          		if (country_name=="India") {
					...
					svg.transition()
          			...
          			return "country_background countries";
          		}
          	})
            .attr("d", path)
            .style("fill", "#eee")
		...
		data.forEach(function(d, i){
			...
			if (!(d.date in date_data)) {
				date_data.push(d.date);
			}
		})
		...
		function repeat_animation() {
			...
			g.selectAll(".cluster_map_date_label")
					.transition()
					.delay(function(d,i) { return i*num_milliseconds_per_date; })
					.duration(step_duration)
						.text(function(d,i){
							cdt = d; 
							console.log(cdt);
							return cdt.getDate() + " " + month_list[cdt.getMonth()];
						});
		...
		*/
		...
		/*
		cdt = start_date;
		repeat_cluster_map_transition();
		function repeat_cluster_map_transition() {
			for (var c=0; c<num_sim_days; c++) {
				...
				g.selectAll(".cluster_map_casecount_label")
					.transition()
					.delay(function(d,i) { return (c-1)*step_delay; })
					.duration(step_duration)
	...
	function zoomed() {
		...
		g.selectAll(".country_focus").style("stroke-width", d3.event.transform.k/20 + "px").style("stroke", "#000");
    }
